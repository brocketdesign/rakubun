    ...
    search_results.forEach((search_result, i) => {
      ...
      const content_promise = moduleCompletion({model: modelGPT, prompt: promptDataContent, max_tokens: 4000})
        .then(fetchContent => {
          ...
          return convertContentHTML + `</br><a href="${search_result.link}" target="_blank">${search_result.title}</a></br>`;
        });
...
async function rsspost(db) {
 ...
 // Content
    ...
    // Title
        ...
        let promise_title = moduleCompletion({model:modelGPT, prompt: promptDataTitle, max_tokens:600})
          .then(fetchTitle=>{
...
// Image
    ...
    let promise_imageGeneration = moduleCompletion({model:modelGPT,role:'stable diffusion prompt generator', prompt:promptDataImage,max_tokens:500})
    .then(fetchPromptImage => {
    return txt2img({prompt:fetchPromptImage,negativePrompt:'',aspectRatio:'5:4',height:816,blogId:blogInfo.blogId});
    })
    .then(imageData => {
