function generatePrompt(data, type) {
    ...
    switch (type) {
        ...
        case '1':
            ...
            result.max_tokens = 30; // Titles are usually short and sweet
            break;
        case '2':
            // Setting up for section heading generation
            const { SECTIONS_COUNT, TITLE: sectionTitle, WRITING_STYLE: sectionStyle, LANGUAGE: sectionLanguage, WRITING_TONE: sectionTone } = data;
            result.prompt = `I’m writing a blog post with the title [${sectionTitle}]. Write ${SECTIONS_COUNT} headings for a blog post outline in ${sectionLanguage}. Style: ${sectionStyle}. Tone: ${sectionTone}. Each heading is between 40 and 60 characters. Use Markdown for the headings (# ).`;
            result.max_tokens = 60; // Approximation for section headings
            break;
        case '3':
